...
contains
    ...
    type(post_processor) function constructor( &
        ...
        save_time_units, post_processor_data_file_name)
        ...
        call constructor%set_properties( &
            manager, post_processor_output_file, operations_number, save_time, &
            save_time_units, output_counter)
    ...
    subroutine read_properties(this, manager, pproc_unit)
        ...
        namelist /pproc_parameters/ post_processor_output_file, operations_number, &
            save_time, save_time_units
    ...
    subroutine set_properties( &
        this, manager, post_processor_output_file, operations_number, save_time, &
        save_time_units, output_counter)
    ...
    subroutine process_data(this, time)
        ...
        ! Work in the configured output-time units, but retain double precision
        ! for all scheduling calculations.
        ...
        ! Compensate only floating-point roundoff. The former 1% tolerance
        ! could trigger an output noticeably before its requested physical time.
        ...
        if ((current_output_time + schedule_tolerance >= target_output_time) .or. &
            ...
            ! Store the actual CFD time of the sampled state. The previous
            ! implementation stored output_counter*save_time, which distorts
            ! the time axis when the solver step exceeds the output interval.
            ...
            ! The integer leading cell remains the anchor for all dependent
            ! operations.  Only the location written to the output is refined.
            do dim = 1, this%domain%get_domain_dimensions()
                this%values(2 + dim) = leading_point_indexes_real(dim)
            end do
            ...
            ! Advance to the first scheduled output instant strictly beyond
            ! the current CFD time. This skips scheduled instants crossed inside
            ! a single solver step and prevents the output counter from lagging.
