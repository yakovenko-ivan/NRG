...
contains
    ...
    type(post_processor_manager) function constructor(manager, number_post_processors)
        ...
        open(newunit=io_unit, file=post_processor_manager_data_file_name, &
    ...
    type(post_processor_manager) function constructor_file(manager)
        ...
        do pproc_counter = 1, size(constructor_file%post_processors)
            constructor_file%post_processors(pproc_counter) = &
                post_processor_c( &
                    manager, &
                    constructor_file%post_processors_setup_file_names(pproc_counter))
        end do
    ...
    subroutine set_properties(this, manager, number_post_processors)
        ...
        do pproc_counter = 1, number_post_processors
            write(this%post_processors_setup_file_names(pproc_counter),'(A,I2,A)') &
                post_processor_data_file_name(1:num_index), pproc_counter, &
                trim(post_processor_data_file_name(num_index+1:))
        end do
    ...
    subroutine write_log(this, log_unit)
        ...
        write(log_unit,'(A,I2)') &
            ' Amount of post processors: ', size(this%post_processors)
    ...
    subroutine write_output_counters(this)
        ...
        do pproc_counter = 1, size(this%post_processors)
            call this%post_processors(pproc_counter)%write_output_counter( &
                this%post_processors_setup_file_names(pproc_counter))
        end do
    ...
    subroutine create_post_processor_operation( &
        this, manager, post_processor_number, field_name, operation_type, &
        operation_area, operation_area_distance, grad_projection, location_mode)
        ...
        if (present(location_mode)) then
            call this%post_processors(post_processor_number)% &
                create_post_processor_operation( &
                    manager, field_name, operation_type, op_area, op_distance, grad_p, &
                    this%post_processors_setup_file_names(post_processor_number), &
                    location_mode)
        else
            call this%post_processors(post_processor_number)% &
                create_post_processor_operation( &
...
end module post_processor_manager_class
